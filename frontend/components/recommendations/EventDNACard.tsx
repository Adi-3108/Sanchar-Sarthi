    ...
    >
      <div className="flex flex-col gap-3">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-2 text-2xl font-semibold text-slate-900">Operational fingerprint</h2>
          <p className="mt-3 text-sm leading-7 text-slate-600">{eventDna.dna_summary}</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <article className="rounded-3xl border border-slate-200 bg-white p-4">
          ...
          <p className="mt-2 text-sm leading-7 text-slate-700">{eventDna.time_context}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-4">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Location context</p>
          <p className="mt-2 text-sm leading-7 text-slate-700">{eventDna.location_context}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-4">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Cause context</p>
          <p className="mt-2 text-sm leading-7 text-slate-700">{eventDna.cause_context}</p>
        </article>
        <article className="rounded-3xl border border-slate-200 bg-white p-4">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Historical pattern</p>
          <p className="mt-2 text-sm leading-7 text-slate-700">{eventDna.historical_pattern}</p>
        </article>
      </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {indicatorEntries.map(([label, value]) => (
            <div key={label} className="rounded-2xl border border-slate-200 bg-white p-3">
              ...
              <p className="mt-2 text-base font-semibold text-slate-900">{formatRiskValue(value)}</p>
            </div>
          ))}
        </div>
      </div>
