# PHASE 2 — Database Schema ORM And Migrations

## Phase Overview

Connect FastAPI to Supabase PostgreSQL and create all core tables.
...
- **Problem being solved:** Connect FastAPI to Supabase PostgreSQL and create all core tables.
...
- **Why now:** This phase depends on Phase 01 and must exist before Data ingestion, features, ML, reports.
...
Deliver this capability: Connect FastAPI to Supabase PostgreSQL and create all core tables.
...
- **New functionality:** Connect FastAPI to Supabase PostgreSQL and create all core tables.
- **New APIs:** GET /api/health database status
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Data ingestion, features, ML, reports
- **New infrastructure:** backend/app/core/database.py; backend/app/db/session.py; backend/app/orm/*.py; backend/alembic/versions/0001_initial_schema.py
- **New data models:** user_accounts, police_officer_profiles, officer_event_assignments, system_audit_logs, events, event_features, event_dna, event_predictions, event_recommendations, hotspot_clusters, citizen_reports, live_event_updates, post_event_reports, model_runs, demo_scenarios, map_api_usage_logs
...
### Canonical Schema Contract For `backend/alembic/versions/0001_initial_schema.py`

The previous inline SQL snippet was removed because it drifted from the real migration and from `docs/Database_Design.md`.

For Phase 02, the executable schema contract is now:

1. `docs/Database_Design.md` Section 3 as the human-readable source of truth.
2. `backend/alembic/versions/0001_initial_schema.py` as the executable Alembic implementation.

Minimum non-negotiable alignment rules:

- `events` must include the extended cleaned ASTraM fields already defined in `docs/Database_Design.md`, including `authenticated`, `modified_datetime`, `reason_breakdown`, `reason_breakdown_clean`, `cargo_material`, `route_path`, `resolved_at_*`, `veh_no_masked`, and `raw_payload`.
- `event_features` must use UUID primary key plus `event_id` foreign key with one-row-per-event uniqueness, and must include derived fields such as `event_hour`, `event_weekday`, `is_weekend`, `is_night_event`, duration fields, `location_cluster_id`, and historical risk/closure-rate columns.
- `event_dna` must use UUID primary key plus `event_id` foreign key with one-row-per-event uniqueness, and must include `dna_summary`, `time_context`, `location_context`, `cause_context`, `weather_context`, `multi_event_context`, `historical_pattern`, `risk_indicators_json`, and `similar_event_ids_json`.
- `hotspot_clusters` must use `location_cluster_id`, centroid fields, cluster rates, `cluster_top_event_cause`, `cluster_risk_score`, and `cluster_profile_json`.
- `event_predictions` must include `model_run_id` plus clearance-range, impact, counterfactual, weather, and multi-event fields already defined in `docs/Database_Design.md`.
- `event_recommendations`, `citizen_reports`, `live_event_updates`, `post_event_reports`, `model_runs`, and `demo_scenarios` must follow the richer table shapes in `docs/Database_Design.md`, not the earlier simplified MVP placeholders.
- `map_api_usage_logs` is part of Phase 02 and must be created here because Phase 14 writes to it.
- Any future schema change must update both `docs/Database_Design.md` and the Alembic migration in the same change set.
...
- **Database entities affected:** user_accounts, police_officer_profiles, officer_event_assignments, system_audit_logs, events, event_features, event_dna, event_predictions, event_recommendations, hotspot_clusters, citizen_reports, live_event_updates, post_event_reports, model_runs, demo_scenarios, map_api_usage_logs
...
- **APIs affected:** GET /api/health database status
...
- **Frontend scope:** Database status display
...
- **Required previous phases:** Phase 01
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Data ingestion, features, ML, reports.
...
- **Required interfaces:** GET /api/health database status
...
```bash
alembic upgrade head; pytest backend/tests/test_database.py
```
