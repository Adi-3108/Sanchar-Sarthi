# PHASE 5 — Hotspot Detection Engine

## Phase Overview

Cluster event locations and produce risk hotspot profiles.
...
- **Problem being solved:** Cluster event locations and produce risk hotspot profiles.
...
- **Why now:** This phase depends on Phase 04 and must exist before Map heatmap, Event DNA, impact scoring.
...
Deliver this capability: Cluster event locations and produce risk hotspot profiles.
...
- **New functionality:** Cluster event locations and produce risk hotspot profiles.
- **New APIs:** GET /api/analytics/hotspots; GET /api/analytics/summary
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Map heatmap, Event DNA, impact scoring
- **New infrastructure:** backend/app/services/hotspot_service.py; backend/app/api/routes_analytics.py; frontend/components/map/HotspotLayer.tsx
- **New data models:** hotspot_clusters, event_features, events
...
### Canonical Hotspot Contract

The completed Phase 05 implementation uses DBSCAN over event coordinates and persists the richer `hotspot_clusters` contract:
...
- `cluster_top_event_cause`
- `cluster_risk_score`
- `cluster_profile_json`

Implementation rules:

- Rebuilds must refresh `event_features` first so `location_cluster_id` and historical fields stay synchronized with hotspot outputs.
...
- **Database entities affected:** hotspot_clusters, event_features, events
...
- **APIs affected:** GET /api/analytics/hotspots; GET /api/analytics/summary
...
- **Frontend scope:** Hotspot map layer
...
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Map heatmap, Event DNA, impact scoring.
...
- **Required interfaces:** GET /api/analytics/hotspots; GET /api/analytics/summary
...
```bash
pytest backend/tests/test_hotspot_service.py
```
