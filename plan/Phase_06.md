# PHASE 6 — Event DNA And Similar Event Memory

## Phase Overview

Generate operational fingerprints and similar historical event evidence.
...
- **Problem being solved:** Generate operational fingerprints and similar historical event evidence.
...
- **Why now:** This phase depends on Phase 04, Phase 05 and must exist before Impact scoring, recommendations, event detail UI.
...
Deliver this capability: Generate operational fingerprints and similar historical event evidence.
...
- **New functionality:** Generate operational fingerprints and similar historical event evidence.
- **New APIs:** Included in GET /api/events/{event_id} and POST /api/events/simulate
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Impact scoring, recommendations, event detail UI
- **New infrastructure:** backend/app/services/event_dna_service.py; backend/app/services/similar_event_service.py; frontend/components/recommendations/EventDNACard.tsx; frontend/components/recommendations/SimilarEventMemoryPanel.tsx
- **New data models:** event_dna, events, event_features, hotspot_clusters
...
- backend/app/services/event_dna_service.py
- backend/app/services/similar_event_service.py
- frontend/components/recommendations/EventDNACard.tsx
- frontend/components/recommendations/SimilarEventMemoryPanel.tsx


## Implementation Code Snippets

### Dataset-Specific Event DNA Rule

Event DNA must not depend on raw `description` text because the dataset contains Kannada/non-English descriptions. Use structured fields first:
...
- time features
- hotspot features
- historical closure/high-priority rates

Use `description_for_features` only when `description_normalization_method` is not `skipped_low_confidence`. Raw Kannada text is preserved for audit/debug but is not used directly in similarity scoring.

### Canonical Event DNA Contract

The completed Phase 06 implementation persists the richer `event_dna` schema rather than the earlier `dna_vector` placeholder:
...
- Structured ASTraM fields remain primary. Normalized description text is only a supporting signal and must contribute `0.0` when `description_normalization_method == 'skipped_low_confidence'`.
...
- **Database entities affected:** event_dna, events, event_features, hotspot_clusters
...
- **APIs affected:** Included in GET /api/events/{event_id} and POST /api/events/simulate
...
- **Frontend scope:** EventDNACard, SimilarEventMemoryPanel
...
- **Required previous phases:** Phase 04, Phase 05
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Impact scoring, recommendations, event detail UI.
...
- **Required interfaces:** Included in GET /api/events/{event_id} and POST /api/events/simulate
...
```bash
pytest backend/tests/test_event_dna.py backend/tests/test_similar_events.py
```
