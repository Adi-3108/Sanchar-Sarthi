...
Generate manpower, barricade, diversion, emergency corridor, logistics impact, and confidence ledger.
...
- **Problem being solved:** Generate manpower, barricade, diversion, emergency corridor, logistics impact, and confidence ledger.
...
- **Why now:** This phase depends on Phase 08 and must exist before Weather, reports, map intelligence, post-event learning.
...
Deliver this capability: Generate manpower, barricade, diversion, emergency corridor, logistics impact, and confidence ledger.
...
- backend/app/services/manpower_service.py
- backend/app/services/barricade_service.py
...
# `impact_category` here must use the shared operational taxonomy:
# Low, Medium, High, Critical.
# Do not use the older low/moderate/high/severe variant.


def recommend_manpower(data: RecommendationInput) -> dict[str, object]:
    ...
    if data.event_type == 'unplanned':
        base += 2
    return {
        'recommended_personnel': base,
        'deployment_style': 'ring_control' if data.impact_score >= 50 else 'point_control',
        'reason_codes': ['impact_score', 'event_type', 'estimated_radius'],
    }


def recommend_barricades(data: RecommendationInput) -> dict[str, object]:
    level = 'soft_channelization'
    if data.road_closure_likelihood >= 0.7:
        level = 'partial_closure_with_barricades'
    if data.impact_score >= 80:
        level = 'controlled_entry_exit_points'
    return {
        'barricade_level': level,
        'coverage_radius_km': data.estimated_radius_km,
        'reason_codes': ['road_closure_likelihood', 'impact_category'],
    }


def generate_recommendation(data: RecommendationInput) -> dict[str, object]:
    return {
        'manpower_plan': recommend_manpower(data),
        'barricade_plan': recommend_barricades(data),
        'diversion_plan': {
            'strategy': 'avoid_hotspot_radius',
            'corridor_to_protect': data.corridor,
            'note': 'Simplified diversion recommendation; not a city-scale routing guarantee.',
        },
        'emergency_corridor_plan': {
            'priority': 'keep_one_lane_clear',
            'authentication_note': 'Ambulance verification is future scope; MVP protects corridor advisory only.',
        },
        'confidence_ledger': [
            {'input': 'ASTraM historical events', 'confidence': 0.85},
            {'input': 'ML/rule impact estimate', 'confidence': 0.65},
            {'input': 'live traffic speed', 'confidence': 0.0, 'note': 'Not present in dataset'},
        ],
    }
```
...
from app.services.recommendation_orchestrator import RecommendationInput, generate_recommendation
...
def generate_event_plan(payload: EventPlanRequest, db: Session = Depends(get_db)) -> dict[str, object]:
    event = db.get(Event, payload.event_id)
    if not event:
        raise HTTPException(status_code=404, detail='Event not found')
    prediction = (
        db.query(EventPrediction)
        .filter(EventPrediction.event_id == payload.event_id)
        .order_by(EventPrediction.created_at.desc())
        .first()
    )
    if not prediction:
        raise HTTPException(status_code=404, detail='Prediction not found for event')
    return generate_recommendation(RecommendationInput(
        impact_score=float(prediction.estimated_impact_score or 0),
        impact_category=prediction.impact_category or 'Low',
        road_closure_likelihood=float(prediction.road_closure_probability),
        event_type=event.event_type or 'unplanned',
        corridor=event.corridor,
        estimated_radius_km=float(prediction.impact_radius_km or 1.5),
    ))
```
---
...
- **APIs affected:** POST /api/recommendations/event-plan
...
- **Frontend scope:** ManpowerPlanPanel, BarricadePlanPanel, DiversionPlanPanel, EmergencyCorridorPanel, FlipkartLogisticsImpactPanel, ActionConfidenceLedger
