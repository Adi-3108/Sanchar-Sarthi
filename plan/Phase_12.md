# PHASE 12 — Live Escalation Simulator

## Phase Overview

Convert live updates into alert levels and adaptive actions.
...
- **Problem being solved:** Convert live updates into alert levels and adaptive actions.
...
- **Why now:** This phase depends on Phase 11 and must exist before Post-event learning.
...
Deliver this capability: Convert live updates into alert levels and adaptive actions.
...
- **New functionality:** Convert live updates into alert levels and adaptive actions.
- **New APIs:** POST /api/events/{event_id}/live-update
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Post-event learning
- **New infrastructure:** backend/app/services/live_escalation_service.py; backend/app/api/routes_live_updates.py; frontend/components/recommendations/LiveEscalationTimeline.tsx
- **New data models:** live_event_updates, citizen_reports, event_predictions
...
- backend/app/services/live_escalation_service.py
- backend/app/api/routes_live_updates.py
- frontend/components/recommendations/LiveEscalationTimeline.tsx
...
def apply_live_update(data: LiveUpdateInput) -> dict[str, object]:
    delta = 0.0
    if data.road_closure_active:
        delta += 10
    if data.officer_shortage:
        delta += 8
    if data.crowd_increase:
        delta += 7
    if data.rain_waterlogging:
        delta += 10
    if data.new_nearby_incident:
        delta += 12

    current_score = min(data.expected_impact_score + delta, 100)
    level = escalation_level(current_score)
    actions = []
    if level in {'Warning', 'Critical'}:
        actions.append('notify_control_room')
        actions.append('prepare_additional_manpower')
    if data.road_closure_active or level == 'Critical':
        actions.append('activate_diversion_plan')
    return {
        'expected_impact_score': round(data.expected_impact_score, 2),
        'current_impact_score': round(current_score, 2),
        'impact_deviation': round(current_score - data.expected_impact_score, 2),
        'alert_level': level,
        'adaptive_action': ', '.join(actions) if actions else 'Monitor situation',
        'honesty_note': 'Simulated live escalation, not automatic police dispatch.',
    }
```
...
@router.post('/api/events/{event_id}/live-update')
def simulate_live_update(event_id: str, payload: LiveUpdateRequest) -> dict[str, object]:
    expected_impact_score = load_expected_impact_score_for_event(event_id)
    return apply_live_update(LiveUpdateInput(
        **payload.model_dump(),
...
- **Database entities affected:** live_event_updates, citizen_reports, event_predictions
...
- **APIs affected:** POST /api/events/{event_id}/live-update
...
- **Frontend scope:** LiveEscalationTimeline
...
- **Required previous phases:** Phase 11
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Post-event learning.
...
- **Required interfaces:** POST /api/events/{event_id}/live-update
...
```bash
pytest backend/tests/test_live_escalation.py
```
