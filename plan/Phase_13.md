...
- **Problem being solved:** Detect simultaneous event conflicts across time, space, diversions, and manpower.
...
- **Why now:** This phase depends on Phase 08, Phase 09 and must exist before Map conflict overlays and coordinated demo.
...
Deliver this capability: Detect simultaneous event conflicts across time, space, diversions, and manpower.
...
- **New functionality:** Detect simultaneous event conflicts across time, space, diversions, and manpower.
- **New APIs:** POST /api/events/multi-event-analysis
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Map conflict overlays and coordinated demo
- **New infrastructure:** backend/app/services/multi_event_service.py; backend/app/api/routes_events.py; frontend/components/recommendations/MultiEventConflictPanel.tsx
- **New data models:** events, event_predictions, event_recommendations
...
- backend/app/services/multi_event_service.py
- backend/app/api/routes_events.py
- frontend/components/recommendations/MultiEventConflictPanel.tsx
...
def time_overlap_minutes(left: EventCoordinationInput, right: EventCoordinationInput) -> float:
    left_end = left.end_datetime or (left.start_datetime + timedelta(hours=2))
    right_end = right.end_datetime or (right.start_datetime + timedelta(hours=2))
    overlap_start = max(left.start_datetime, right.start_datetime)
    overlap_end = min(left_end, right_end)
    return max((overlap_end - overlap_start).total_seconds() / 60, 0)


def detect_pair_conflict(left: EventCoordinationInput, right: EventCoordinationInput, available_personnel: int) -> dict[str, object]:
    overlap = time_overlap_minutes(left, right)
    distance = haversine_km(left.latitude, left.longitude, right.latitude, right.longitude)
    same_corridor = bool(left.corridor and left.corridor == right.corridor)
    ...
    return {
        ...
        'conflict_level': 'high' if score >= 70 else 'medium' if score >= 40 else 'low',
...
### Multi-Event Coordination Output
```python
def summarize_conflicts(conflicts: list[dict[str, object]]) -> dict[str, object]:
    high = [item for item in conflicts if item['conflict_level'] == 'high']
    return {
        'coordination_mode': 'joint_command' if high else 'normal_monitoring',
        'high_conflict_count': len(high),
        'recommended_action': 'separate diversion corridors and reserve mobile units' if high else 'monitor overlaps',
        'conflicts': conflicts,
    }
```
---
...
- **Database entities affected:** events, event_predictions, event_recommendations
...
- **APIs affected:** POST /api/events/multi-event-analysis
...
- **Frontend scope:** MultiEventConflictPanel
...
- **Required previous phases:** Phase 08, Phase 09
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Map conflict overlays and coordinated demo.
...
- **Required interfaces:** POST /api/events/multi-event-analysis
...
```bash
pytest backend/tests/test_multi_event_service.py
```
