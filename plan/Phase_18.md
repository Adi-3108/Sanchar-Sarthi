# PHASE 18 — Deployment Monitoring Final QA And Submission Readiness

## Phase Overview

Deploy on free tiers, monitor health, and verify full demo.
...
- **Problem being solved:** Deploy on free tiers, monitor health, and verify full demo.
...
- **Why now:** This phase depends on Phase 17 and must exist before Submission.
...
Deliver this capability: Deploy on free tiers, monitor health, and verify full demo.
...
- **New functionality:** Deploy on free tiers, monitor health, and verify full demo.
- **New APIs:** All APIs smoke tested
- **New workflows:** The product moves forward in the end-to-end traffic command workflow.
- **New capabilities:** Submission
- **New infrastructure:** .github/workflows/ci.yml; backend/Dockerfile; frontend/vercel.json; README.md
- **New data models:** All tables migrated and seeded
...
url = sys.argv[1]
response = httpx.get(f'{url.rstrip("/")}/api/health', timeout=10)
response.raise_for_status()
payload = response.json()
assert payload['status'] == 'ok'
print('EventFlow API healthy')
```
---

## Database Requirements

- **Database entities affected:** All tables migrated and seeded
...
- **APIs affected:** All APIs smoke tested
...
- **Frontend scope:** Deployed frontend
...
- **Required previous phases:** Phase 17
- **Integration method:** This phase reuses previous contracts and creates outputs consumed by Submission.
...
- **Required interfaces:** All APIs smoke tested
...
```bash
npm run build; pytest; docker compose up --build; curl /api/health
```
